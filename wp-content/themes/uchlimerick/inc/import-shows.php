                            ...
                            if ( $shows_count ) {
                                ...
                                if ( $shows ) {
                                    foreach ( $shows as $key=> $single_prop ) {
                                        ...
                                        if ( isset( $meta_values['tags'] ) && ! empty( $meta_values['tags'] ) ) {
                                            if(is_array($meta_values['tags']) && count($meta_values['tags'])==1 ){
                                                $tags = reset( $meta_values['tags'] );
                                            }
                                            else{
                                                $tags =$meta_values['tags'];
                                            }
                                            if(is_array($tags)){
                                                foreach ( $tags as $tag_name ) {
                                                    $this->check_assign_terms( $post_id, $tag_name, 'genre' );
                                                }
                                                $meta_values['tags'] = $tags;
                                            }
                                            else{
